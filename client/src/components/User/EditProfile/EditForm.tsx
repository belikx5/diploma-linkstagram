...
import {
  ...
  deleteDeadProfile,
  updateDeadProfile,
} from "../../../store/actions/deadProfileActions";
...
const EditForm = ({ openModal }: EditFormProps) => {
  ...
  const performDeadProfileAction = (): Promise<void> => {
    //if is dead flag changed => create/update/delete dead profile
    if (currentUser && currentUser.is_dead_profile !== isDead) {
      if (isDead && !deadProfile)
        return dispatch(createDeadProfile(currentUser.id, trustedUsers));
      else
        return deadProfile
          ? dispatch(deleteDeadProfile(deadProfile.id))
          : Promise.resolve();
    } else if (isDead && deadProfile)
      return dispatch(updateDeadProfile(deadProfile.id, trustedUsers));
